<template>
  <div id="sequencerBlock">
    ...
    <context-menu id="context-menu1" ref="ctxMenu">
			...
			<li class="ctx-item" @click="saveCustomSequencer"> Save </li>
		</context-menu>
...
export default {
  ...
  methods:{
    renameBlockNameProp: function(){
      var self = this
      this.$swal({  text: 'Rename the sequencer', 
                    input: 'text', 
    ...
    saveCustomSequencer: function(){
      if(this.blockNameProp == "Sequencer"){
        this.$swal({icon: 'error', text: 'Block must be renamed before save to repository'})
        //window.console.log(this.$root.$children[0].$children[0])
      }
      else{
        var i;
